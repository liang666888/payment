...
public class OkHttpUtil {
	...
	private static String execNewCall(Request request) {
		...
		try {
			...
			throw new BizException(PayApiRetCodeEnum.SYSTEM_TIMEOUT.code, PayApiRetCodeEnum.SYSTEM_TIMEOUT.msg);
		} finally {
			if (response != null) {
				response.close();
			}
	 ...
	 * @return
	 */
	public static String certifyPost(String url, Map<String, String> params, String token) {
		FormBody.Builder builder = new FormBody.Builder();
		// 添加参数
		if (params != null && params.keySet().size() > 0) {
			for (String key : params.keySet()) {
		...
		Request request = new Request.Builder().url(url).addHeader("token", token).post(builder.build()).build();
		return execNewCall(request);
	}
}